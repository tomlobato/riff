      ...
      def initialize(rack_req_headers, ip_spoofing_check = true, custom_proxies = nil)
        ...
        @proxies =
          if custom_proxies.blank?
            ...
            raise(ArgumentError, "custom_proxies must be an array of IPAddr instances.")
          end
      end
        ...
        def calculate_ip
          ...
          if should_check_ip && !forwarded_ips.include?(client_ips.last)
            raise(
              IpSpoofAttackError,
              "IP spoofing attack?! " \
              "HTTP_CLIENT_IP=#{@headers['HTTP_CLIENT_IP'].inspect} " \
              "HTTP_X_FORWARDED_FOR=#{@headers['HTTP_X_FORWARDED_FOR'].inspect}"
            )
          end
          ...
          filter_proxies((ips + [remote_addr]).compact).first || ips.last || remote_addr
        end
